...
window.addEventListener('scroll', _.throttle(()=>{
  ...
  //_.throttle : badge를 보이거나 안 보이게 처리
  //스크롤 값이 500이상이면 안보이고, 이하이면 보이게 
  if(window.scrollY > 500){
    // badgeEl.style.display = 'none';
    // gsap은 lib, 동작(애니메이션)
    // gsap.to(요소, 지속시간, {옵션})
    gsap.to(badgeEl, .6, { /* gsap 부드럽게 사라지고 보이게함 */
      opacity:0,
      display:'none'
    });
    gsap.to(toTopEl, .7, {
...
function random(min, max){
  ...
  // parseFloat()을 통해 소수점을 가져오는 숫자 데이터로 반환
...
spyEls.forEach(function(spyEl){
  ...
  .setClassToggle(spyEl, 'show') //이 위치가 되면 show라는 class를 넣음 -> 보여줌(안으로 들어오는 구조를 만듦)
